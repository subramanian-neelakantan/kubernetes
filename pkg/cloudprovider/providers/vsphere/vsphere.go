...
func init() {
	...
	cloudprovider.RegisterCloudProvider(ProviderName, func(config io.Reader) (cloudprovider.Interface, error) {
		klog.V(4).Infof("vSphere cloud provider init called...")
		// If vSphere.conf file is not present then it is worker node.
		if config == nil {
			klog.V(4).Infof("Creating a worker node")
		...
		klog.V(4).Infof("Creating a controller node")
...
func (vs *VSphere) Initialize(clientBuilder cloudprovider.ControllerClientBuilder, stop <-chan struct{}) {
	klog.V(4).Infof("Client builder Initialize called")
}
...
func buildVSphereFromConfig(cfg VSphereConfig) (*VSphere, error) {
	...
	klog.V(4).Infof("Created a new NodeManager %v, %v", vs.nodeManager, vs.nodeManager)
	klog.V(4).Infof("For vSphere instance %p, %v", vs, vs)
	return &vs, nil
}
...
func (vs *VSphere) GetVolumeLabels(volumePath string) (map[string]string, error) {
	...
	if len(dsZones) > 0 {
		labels[kubeletapis.LabelZoneRegion] = dsZones[0].Region
		labels[kubeletapis.LabelZoneFailureDomain] = dsZones[0].FailureDomain
	}
	return labels, nil
}
